...
const Modal = ({open, onClose, children}: {open:boolean, onClose:MouseEventHandler<HTMLElement>, children:React.ReactNode}) => {
  return (
	// backdrop
	<div onClick={onClose} className={`fixed inset-0 flex justify-center items-center transition-colors ${open ? 'visible bg-black/20' : 'invisible'}`}>
		{/* card */}
		<div onClick={(e) => e.stopPropagation()} className={`bg-white rounded-xl shadow p-10 w-96 transition-all ${open ? 'scale-100 opacity-100' : 'scale-125 opacity-0'}`}>
